...
// 🔥 SUBSTITUA pelos dados do SEU NOVO projeto Firebase
// (crie um projeto separado em https://console.firebase.google.com,
//  ative o Firestore Database, e copie a config em
//  Configurações do projeto ⚙️ → Seus apps → Web)
...
function renderTable() {
  ...
  thead.innerHTML = '<th class="td-collab">Colaborador</th>' +
    ...
    '<th>Total</th><th>Reprov.</th><th>Final.</th>';
  ...
  analistasToShow.forEach(function(analista) {
    ...
    dates.forEach(function(d) {
      ...
      if (!e) {
        ...
        tdHtml += '<td><div class="cell-hs"><span class="cell-r">'+(e.reprovada||0)+'R</span><span class="cell-f">'+(e.finalizada||0)+'F</span></div>'
  ...
  tfoot.innerHTML =
    ...
    '<tr><td>Reprov.</td>'+totByDate.map(function(x){ return '<td class="cell-r">'+x.reprovada+'</td>'; }).join("")+'<td colspan="3"></td></tr>'+
    '<tr><td>Final.</td>'+totByDate.map(function(x){ return '<td class="cell-f">'+x.finalizada+'</td>'; }).join("")+'<td colspan="3"></td></tr>';
}
